...
## Epic Goal
Initialize the "Wind Manager Dashboard" using Evidence.dev, connect it to the existing Supabase database (containing wind turbine data), and establish a continuous deployment pipeline to Cloudflare Pages. This serves as the foundation for future data visualizations.

## Epic Description

**Existing System Context:**
- **Functionality:** The system aggregates wind turbine data (currently in Supabase) from various sources (APIs, scraping).
- **Technology Stack:** Supabase (PostgreSQL), Evidence.dev (New Frontend), Cloudflare Pages (New Hosting).
- **Integration Points:** Evidence connects directly to Supabase via connection string.

**Enhancement Details:**
- **What's being added:** A new Evidence project repository, configuration for Supabase connectivity, and a CI/CD pipeline to Cloudflare.
- **How it integrates:** Evidence queries Supabase in build time (or run time depending on mode) to generate static sites deployed to Cloudflare.
- **Success Criteria:** 
  1. A local Evidence app runs and displays data from Supabase.
  2. The app is publicly accessible via a Cloudflare Pages URL.
...
### Story 2: Deploy to Cloudflare Pages
**Description:** Configure Cloudflare Pages to build and deploy the Evidence project.
**Acceptance Criteria:**
- Build settings are configured in Cloudflare (build command `npm run build`, output dir `build/` or similar).
- Environment variables (Supabase connection) are securely added to Cloudflare.
...
## Risk Mitigation
- **Primary Risk:** Connection issues or build timeouts on Cloudflare.
- **Mitigation:** Test build command locally first (`npm run build`). Use Cloudflare logs to debug.
- **Rollback Plan:** Revert git commits; delete Cloudflare project if needed.

## Definition of Done
- [ ] Evidence project committed to repo.
- [ ] Local build passes.
- [ ] Cloudflare deployment is green.
